import { getLiveCattle, friendlyErrorMessage } from "./data-service.js";
...
async function init() {
  try {
    const items = await getLiveCattle();

    if (items.length === 0) {
      status.textContent = "No cattle listings available right now — check back soon.";
      return;
    }
